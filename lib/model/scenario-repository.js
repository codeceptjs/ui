...
const debug = Debug('codeceptjs:scenario-repository');
import fs from 'fs';
import path from 'path';
import chokidar from 'chokidar';

import throttled from './throttling.js';
import codeceptjsFactory from './codeceptjs-factory.js';
...
const watcher = chokidar.watch(watchedFiles(), {
  ...
  // Emit detailed file change notification
  wsEvents.codeceptjs.fileChanged(fileRelPath, event);
  
  // Check if config file changed
  if (fileRelPath === codeceptjsFactory.getConfigFile()) {
    wsEvents.codeceptjs.configUpdated(fileRelPath);
  }
...
export default {
  ...
  /** Close the file watcher to release resources */
  closeWatcher() {
    if (watcher) {
      watcher.close();
    }
