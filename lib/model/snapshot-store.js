...
const extractUrlWithPath = str => {
  ...
  return `${parsed.protocol}//${parsed.host}${parsed.port ? ':' : ''}${parsed.port ? parsed.port : ''}/${pathname}`;
}
...
const fixHtmlSnapshot = step => {
  if (step.snapshot && step.snapshot.sourceContentType === 'html') {
    ...
    processedSource = processedSource.replace(/href="([^h][^t][^t][^p][^/]+)/gi, `src="${pageUrlWithPath}/$1`);
