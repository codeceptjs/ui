...
const debug = Debug('codeceptjs:reporter-utils');
...
const takeSnapshot = async (helper, snapshotId, takeScreenshot = false, retry = 3) => {
  ...
  let pageUrl, pageTitle, scrollPosition, viewportSize;

  try {
    // eslint-disable-next-line no-unused-vars
    const [screenshot, ...rest] = await Promise.all([
    ...
    [pageUrl, pageTitle, scrollPosition, viewportSize] = rest;
    ...
    if (retry > 0) {
      ...
      return takeSnapshot(helper, snapshotId, takeScreenshot, retry - 1);
    }
