...
module.exports = new class CodeceptjsFactory {
  ...
  async create(cfg = {}, opts = {}) {
    ...
    // Load configuration with proper hook processing
    const configPath = path.join(TestProject, this.getConfigFile());
    try {
      // First, execute the config file to ensure hooks are applied
      delete require.cache[require.resolve(configPath)];
      require(configPath);
    } catch (err) {
      debug('Could not pre-execute config file:', err.message);
    }
  ...
  getCodeceptjsConfig() {
    try {
      ...
      // Return a default config when CodeceptJS is not initialized
      return {
        tests: './',
        timeout: 10000,
        output: './output',
