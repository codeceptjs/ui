...
class RealtimeReporterHelper extends Helper {
      ...
      if (retvalPromise) {
        try {
          ...
          if (retval) {
            ...
            console.log('STEP-PASSED', retval);
          }
  ...
  async _before(t) {
    ...
    wsEvents.rtr.testBefore(mapSuiteAndTest(this.test.startedAt, this.suite, this.test));
  }

  async _after(t) {
    if (!this._isEnabled(t)) return;

    wsEvents.rtr.testAfter(mapSuiteAndTest(Date.now(), this.suite, this.test));
  }
