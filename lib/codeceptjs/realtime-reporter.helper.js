...
const debug = Debug('codeceptjs:realtime-reporter-helper');
...
import { toError, isScreenshotStep, takeSnapshot, filterStack, grabSource } from './reporter-utils.js';
...
/**
 * Factory that creates RealtimeReporterHelper class.
 * Receives the Helper base class from the factory (loaded via ESM import).
 * @param {Function} HelperBase - The CodeceptJS Helper base class
 * @returns {RealtimeReporterHelper} instance
 */
export default function createRealtimeReporterHelper(HelperBase) {
  let event, output, recorder, store;

  // These will be set from the cached codeceptjsModules
  const modulesPromise = codeceptjsModules().then(mods => {
    event = mods.event;
    output = mods.output;
   ...
   */
  class RealtimeReporterHelper extends HelperBase {
    constructor(options) {
      super(options);
      ...
      // Event listeners are registered once modules are loaded
      modulesPromise.then(() => this._registerEventListeners()).catch(err => {
        debug('Failed to load CodeceptJS modules for event listeners:', err.message);
      });
    }
    ...
    _registerEventListeners() {
      ...
      process.on('unhandledRejection', (err) => {
        output.error('Unhandled rejection', err.stack);
        if (!this.test) return;
        wsEvents.rtr.testFailed({
  ...
  return new RealtimeReporterHelper();
}
