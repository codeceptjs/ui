...
const fixHtmlSnapshot = step => {
  if (step.snapshot && step.snapshot.sourceContentType === 'html') {
    ...
    processedSource = processedSource.replace(/<img.*src="([^/h])/gi, `src="${url.protocol}//${url.hostname}${url.port ? ':' + url.port : ''}${url.pathname}$1`);
    processedSource = processedSource.replace(/<img.*src="\/\//gi, `src="https://`);
    processedSource = processedSource.replace(/<img.*src="\//gi, `src="https://${url.hostname}/`);
