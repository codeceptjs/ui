...
import { Suite, Test, reporters } from 'mocha';
...
export default async function (workers, config, options, codecept, container) {
  const { satisfyNodeVersion, getTestRoot } = await importSubpath('codeceptjs/lib/command/utils');
  const Codecept = await importSubpath('codeceptjs/lib/codecept');
  const output = await importSubpath('codeceptjs/lib/output');
  const event = await importSubpath('codeceptjs/lib/event');
  const runHook = await importSubpath('codeceptjs/lib/hooks');
