...
const debug = Debug('codeceptjs:console-recorder-helper');
import wsEvents from '../model/ws-events.js';
import { v4 as uuid } from 'uuid';

/**
 * Factory that creates ConsoleRecorderHelper class.
 * @param {Function} HelperBase - The CodeceptJS Helper base class
 * @returns {ConsoleRecorderHelper} instance
 */
export default function createConsoleRecorderHelper(HelperBase) {
  class ConsoleRecorderHelper extends HelperBase {
    constructor(options) {
      super(options);
    }
  ...
  return new ConsoleRecorderHelper();
}
