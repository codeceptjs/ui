...
module.exports = async (req, res) => {
  ...
  try {
    docsWebApiFolderPath = path.join(path.dirname(require.resolve('codeceptjs')), '/../docs/webapi');
  } catch (e) {
    // In ESM mode, require.resolve may not work; try a fallback
    const codeceptjsPath = require.resolve('codeceptjs/package.json');
    docsWebApiFolderPath = path.join(path.dirname(codeceptjsPath), 'docs/webapi');
  }
  const docFileList = [];
  try {
