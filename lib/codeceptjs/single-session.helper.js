import { getSettings } from '../model/settings-repository.js';
 ...
 */
export default function createSingleSessionHelper(HelperBase) {
  // container will be loaded lazily from codeceptjs modules
  let container;
  codeceptjsModules().then(mods => {
    container = mods.container;
  }).catch(err => {
    // Container loading failed - _init() will be a no-op
    console.error('Failed to load CodeceptJS container:', err.message); // eslint-disable-line no-console
  });
