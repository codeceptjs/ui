...
const mapStep = (id, s, snapshot, returnValue) => {
  assert(id, 'id is required');
  assert(s, 'step is required');

  return {
    id,
    duration: (Date.now() - this.stepStartedAt), // TODO: Use already available step.endTime - step.startTime
    actor: 'I',
...
class RealtimeReporterHelper extends Helper {
  constructor(options) {
    ...
    event.dispatcher.on('step.passed', async (step, retvalPromise) => {
      ...
      console.log('step.passed', step.name, retval);
      socket.emit('step.passed', mapStep(this.stepIndex, step, undefined, retval));
    });
  }
  ...
  async _emitSnapshot(helper, s, takeScreenshot = false) {
    ...
    socket.emit('step.before', mapStep(this.stepIndex, s, snapshot, returnValue));
